...
/**
 * Every search-record type the site is allowed to index.
 *
 * Held once, because it was previously written out inside two tests and the copies drifted the
 * moment a feature added a surface. A new type belongs here deliberately: the point of the list is
 * that an unsanctioned record — a drill item above all — can never reach the index unnoticed.
 *
 * Feature 002 added its own surfaces, whose item text is not indexed because it travels in
 * `data-pagefind-ignore` script payloads. Feature 003 added the three Claude Code surfaces.
 */
...
test.describe("User Story 6: Format demonstration exclusion from search", () => {
  ...
  for (const term of uniqueExcludedDrillTerms) {
    test(`searching "${term}" never surfaces the excluded drill item`, async ({
      page,
    }) => {
  ...
  test("searching overlapping terms from drill item never returns format demonstration content", async ({
    ...
    for (let i = 0; i < count; i++) {
      ...
      const itemType = await item.getAttribute("data-type");
      if (itemType) {
        expect(ALLOWED_SEARCH_TYPES.has(itemType)).toBe(true);
        expect(itemType).not.toBe("drill");
      }
