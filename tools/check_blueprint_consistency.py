...
def check_site_mock_data(blueprint_path: Path, data_path: Path) -> None:
    """Require generated mock data, when present, to equal a fresh export from the blueprint.

    Unlike ``blueprint.json`` this file is generated and git-ignored, regenerated by the site's
    ``prebuild`` on every build, so a clean checkout legitimately has none yet. What must never
    happen is a file that exists and disagrees with ``BLUEPRINT.md`` — a hand edit, or a blueprint
    change without a rebuild — and that is what this rejects.
    """
    ...
    try:
        ...
        raise ConsistencyError(f"Derived site mock data cannot be read: {data_path}.") from error
    try:
        expected = export_mock_data(blueprint_path)
    except MockDataExportError as error:
        raise ConsistencyError(f"Derived site mock data cannot be generated: {error}") from error
    if actual != expected:
        raise ConsistencyError("Derived site mock data is stale against BLUEPRINT.md.")
